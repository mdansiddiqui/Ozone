...
{
    public class ProjectGeneralFormService : GenericRepositoryAsync<ClientProjects>, IProjectGeneralFormService
    {
        ...
        public ProjectGeneralFormService(
        ...
        //public async Task<string> Create(ProjectGeneralFormModel input)
        //{
        ...
        //    var ClientProjectMod = new ClientProjectModel();
        //    var ClientSitesModel = new ClientSitesModel();
        //    //var Client = _dbContext.ClientProjects.Include(x => x.ClientId).Where(x => x.ClientId == 58).FirstOrDefault();
        ...
        public async Task<List<smetaauditPillarsModel>> GetAllSmetaAuditPillars()
        {
            var result = new List<smetaauditPillarsModel>();

            //var list = await Task.Run(() => _dbContext.SmetaauditPillars.Where(x => x.IsDeleted == false).ToList());
           // result = _mapper.Map<List<smetaauditPillarsModel>>(list);
            return result;
        }


        public async Task<string> ProjectGeneralFormDeleteById(long id)
        {
            ...
            using (var transaction = _unitOfWork.BeginTransaction())
            {
                ...
                ProjectGeneralForm dbresult = _dbContext.ProjectGeneralForm.Where(u => u.ClientProjectId == id).FirstOrDefault();
                ...
                if (dbresult != null)
                {
                    ...
                    try
                    {
                        ...
                        Project.ProjectGeneralForm.Add(dbresult);
        ...
        public async Task<ProjectGeneralFormModel> DownloadFile(long id)
        {
            // OzoneContext ozonedb = new OzoneContext();
            using (var transaction = _unitOfWork.BeginTransaction())
            {
                ProjectGeneralForm DbLibrary = await Task.Run(() => _dbContext.ProjectGeneralForm.Where(x => x.ClientProjectId == id).FirstOrDefault());

                ProjectGeneralFormModel Li = new ProjectGeneralFormModel();
                if (DbLibrary != null)
                {
        ...
        public async Task<string> SubmitForReview(long id, long loginUserId)
        {
            ...
            using (var transaction = _unitOfWork.BeginTransaction())
            {
                ...
                ProjectGeneralForm Sa = _dbContext.ProjectGeneralForm.Where(u => u.ClientProjectId == id).FirstOrDefault();
                ...
                if (ClientProject != null)
                {
                    if (Sa.ApplicationFormPath != null && Sa.ApplicationFormPath != "")
                    {
                        ...
                        ClientProject.ProjectGeneralForm.Add(Sa);
                        await base.UpdateAsync(ClientProject);
                        await _unitOfWork.SaveChangesAsync();
