import { DatePipe } from '@angular/common';
import { Component, Input, OnInit } from '@angular/core';
import { FormControl, FormGroup } from '@angular/forms';
import { ActivatedRoute, Router } from '@angular/router';
import { PagedRequestModel } from '@shared/Dto/paged-Request-model';
import { ClientService } from '@shared/Services/Client-Service';
import { MakerAuthorizerFormService } from '@shared/Services/maker-authorizer-form.service';
import { HiggService } from '@shared/Services/project-higg-service';
import { SlcpService } from '@shared/Services/project-slcp-service';
import { UserStandardService } from '@shared/Services/User-Standard-service';
import { ToastrService } from 'ngx-toastr';
import { GeneralFormService } from '@shared/Services/project-general-form-service';
import pdfmake from 'pdfmake/build/pdfmake';
import pdfFonts from "pdfmake/build/vfs_fonts";
import { LibraryResourceService } from '@shared/Services/library-Resource_service';

@Component({
  selector: 'app-general-form',
  templateUrl: './general-form.component.html',
  styleUrls: ['./general-form.component.css']
})
export class GeneralFormComponent implements OnInit {

  GeneralForm = new FormGroup({

    SmetaAuditId: new FormControl(''),
    StandardId: new FormControl(''),
    Code: new FormControl(''),
    ClientSiteId: new FormControl(''),
    LegalStatus: new FormControl(''),
    OutsourceProductionProcessess: new FormControl(''),
    TotalEmployees: new FormControl(''),
    ShiftTimings: new FormControl(''),
    VerificationTypeId: new FormControl(''),
    Address: new FormControl(''),
    //RequestOfSiteId: new FormControl(''),
    ApprovalStatusId: new FormControl(''),
    ContractRemarks: new FormControl(''),
    ContractStatusId: new FormControl(''),
    AuditTypeId: new FormControl(''),
    SiteFactory: new FormControl(''),
    SiteProductionProcess: new FormControl(''),
    ProductCategory: new FormControl(''),
    Ntnnumber: new FormControl(''),
    CertificationId: new FormControl(''),


    // CountryId: new FormControl(''),
    // StateId: new FormControl(''),
    //CityId: new FormControl(''),
    ProjectTypeId: new FormControl(''),
    ServicesTypeId: new FormControl(''),
    //MethodologyId: new FormControl(''),
    //FromDate: new FormControl(''),

    //ToDate: new FormControl(''),
    //ApplicationDate: new FormControl(''),
    //CompletedStepId: new FormControl(''),
    //TentativeDate: new FormControl(''),
    //AssessmentCompletedId: new FormControl(''),
    //CompletedModuleId: new FormControl(''),
    //  AccreditationId: new FormControl(''),

    //NoOfMd: new FormControl(''),
    //ModuleVersionId: new FormControl(''),
    //BodySelectDate: new FormControl(''),
    //ModuleShareId: new FormControl(''),
    // WindowPeriodDate: new FormControl(''),

    //ModuleYear: new FormControl(''),
    Remarks: new FormControl(''),

    //ProjectCode: new FormControl(''),

  })

  get f() { return this.GeneralForm.controls; }

  onSubmit() {
    debugger

    //   this.GeneralForm.get('FromDate').value;
    //   this.GeneralForm.get('ToDate').value;
    //   var  date1 = new Date(this.GeneralForm.get('FromDate').value); 
    //   var date2 = new Date(this.GeneralForm.get('ToDate').value); 

    //     var Time = date2.getTime() - date1.getTime(); 
    //     var Days = Time / (1000 * 3600 * 24);

    //     if(Days<15)
    //     {
    //       abp.message.info("Duration should minimum 15 days between 'From Date' and 'To Date'")
    //     }
    // else{

    //this.submitted = true;

    // stop here if form is invalid
    if (this.GeneralForm.invalid) {
      abp.message.error("Some fields are required ");
      return;
    }

    this.onSubmit1();
    //}
    // if (this.fileToUpload != null && this.fileToUpload != "" && this.fileToUpload != undefined && this.fileToUpload != undefined && this.fileToUpload != NaN) {
    //   this.onSubmit1();
    // }
    // else {

    //   abp.message.error("Please Upload Application Form!", "Application Form is required ");
    // }
    // display form values on success
    //alert('SUCCESS!! :-)\n\n' + JSON.stringify(this.SLCPForm.value, null, 4));
  }

  datePipe = new DatePipe("en-US");

  @Input() formName: string
  submitted = false;
  secRoleForm
  tableSizes = [3, 5, 10, 15, 20, 25];
  public totalCount: number
  public pagedDto: PagedRequestModel = new PagedRequestModel()
  pageNumber: number = 1
  pageSize: number = 10
  public isEditShown: boolean
  public isViewShown: boolean
  public isAddShown: boolean
  public IsContract: boolean = false
  public keyword: string = ''
  public savebtn: boolean = false
  public deletebtn: boolean = false
  public btnApproval: boolean = false
  public btnContract: boolean = false
  public btnContractSave: boolean = false
  public btnback: boolean = true;
  public ApprovalList = [];
  public ContractApprovalList = [];
  public btnContractDownload: boolean = false
  public TransferProjectList = [{ id: "1", code: "Yes" }, { id: "0", code: "No" }];
  public standardId: number
  public ProjectId: number
  public ClientId: number
  public cllientSiteId: number
  public savedownload: boolean = false
  public ClientSiteList = [];
  public VerificationTypeList = [];
  public ProjectTypeList = [];
  public SerivceTypeList = [];
  public MethodologyList = [];
  public AccreditationList = [];
  public CompletedSetupList = [];
  public AssessmentCompletedList = [];
  public ClientSiteData: any;
  public OrganizationId: number
  public RequestOfSiteList = [];
  public ModuleVersionList = [];
  public ModuleShareList = [];
  public CertificationList = [];
  public AuditTypeList = [];
  public SmetaAuditList = [];
  public GeneralList = [];
  public isShown: boolean = false
  public authorizer: boolean = false
  public Inserted: boolean = false
  fileToUpload: any;
  clientinfo: any;
  public UserStatusList = []
  public ProjectCode: string
  public ProjectStatus: string
  public Revieweruser: boolean = false
  readonly allowedPageSizes = [5, 10, 'all'];
  readonly displayModes = [{ text: "Display Mode 'full'", value: "full" }, { text: "Display Mode 'compact'", value: "compact" }];
  displayMode = "full";
  showPageSizeSelector = true;
  showInfo = true;
  showNavButtons = true;
  customizeColumns(columns) {
    columns[0].width = 70;
  }
  get isCompactMode() {
    return this.displayMode === "compact";
  }



  constructor(private _UserStandardService: UserStandardService,
    private _HiggService: HiggService,
    private _ClientService: ClientService,
    private _toster: ToastrService,
    private router: Router,
    private route: ActivatedRoute,
    private _SlcpService: SlcpService,
    private _GeneralFormService: GeneralFormService,
    public LibraryResourceService: LibraryResourceService,
    private _makerAuthorizerFormService: MakerAuthorizerFormService) { }


  ngOnInit(): void {
    
    this.editProject();
    this.CheckReviewer();
    this.loadStatus();
    //this.loadProjectType();
    this.loadVerificationType();
    //this.loadAccreditation()
    this.loadAllApprovalStatus();
    this.loadAllCertification();
    this.loadAuditTypeId();
    this.loadSmetaAudit();
    this.loadClientSites();
    //this.loadServiceType();
    //this.loadRequestOfSite();
    //this.loadSecRoleForm()

    // this.editUser()


    //this.onSearch();

    //this.LoadCompletedSetup();
    //this.loadAssessmentCompleted();
    //this.loadMethodology();

    //this.loadModuleShare();
    //this.loadModuleVersion();

  }
  ngAfterViewInit(): void {

    if (this.ProjectId > 0) {
      this.UpdateProject();
    }




  }
  CheckReviewer() {
    debugger
    const UserModel =

    {

      StandardId: this.standardId,
      UserId: parseInt(localStorage.getItem('userId'))




    };

    this._SlcpService.GetReviewerByStandard(UserModel).subscribe((Response) => {
      debugger
      var reviwerData = Response
      if (reviwerData == null || reviwerData == undefined || reviwerData == "" || reviwerData == '' || reviwerData == NaN) {
        this.Revieweruser = false
        this.authorizer = false
        this.btnApproval = false
      }
      else if (parseInt(reviwerData.userId) == parseInt(localStorage.getItem('userId'))) {
        this.authorizer = true
        this.Revieweruser = true
        this.btnApproval = true
      }
      else { this.Revieweruser = false }
      this.loadSecRoleForm();
    })

  }





  loadSecRoleForm() {
    debugger
    // let secRoleForm = JSON.parse(localStorage.getItem('secRoleForm'))
    // let permission = secRoleForm.find(x => x.formCode != null && x.formCode == this.formCode)

    // this._makerAuthorizerFormService.getSecRoleForm().subscribe((data) => {

    //   let formName = (this.formName == undefined ? localStorage.getItem('formName') : this.formName)
    //   this.secRoleForm = data.find(x => x.formCode != null && x.formCode == this.formName)

    var formName = "GeneralForm"
    this._makerAuthorizerFormService.getSecRoleForm().subscribe((data) => {

      this.secRoleForm = data.find(x => x.formName == formName)
      this.isShown = this.secRoleForm.authAllowed
      if (this.Revieweruser == true) {
        //this.secRoleForm.insertAllowed = false
        this.secRoleForm.authAllowed = true
      }
      else {

        this.secRoleForm.authAllowed = false
      }
      if (this.secRoleForm.insertAllowed == false) {

        //this.btnApproval = true
        //this.authorizer = true
        this.GeneralForm.get('ClientSiteId').disable();
        this.GeneralForm.get('VerificationTypeId').disable();
        this.GeneralForm.get('ProjectTypeId').disable();
        this.GeneralForm.get('StandardId').disable();
        this.GeneralForm.get('ProductCategory').disable();
        this.GeneralForm.get('Ntnnumber').disable();
        this.GeneralForm.get('SiteProductionProcess').disable();
        this.GeneralForm.get('SiteFactory').disable();
        this.GeneralForm.get('AuditTypeId').disable();
        this.GeneralForm.get('SmetaAuditId').disable();
        



        //this.GeneralForm.get('MethodologyId').disable();
        //this.GeneralForm.get('FromDate').disable();
        //this.GeneralForm.get('ToDate').disable();
        //this.GeneralForm.get('ApplicationDate').disable();
        //this.GeneralForm.get('AssessmentCompletedId').disable();
        //this.GeneralForm.get('AssessmentCompletedId').disable();
        // this.SLCPForm.get('CompletedModuleId').disable();
        //this.GeneralForm.get('CompletedStepId').disable();
        //this.GeneralForm.get('TentativeDate').disable();
        //this.GeneralForm.get('NoOfMd').disable();
        //this.GeneralForm.get('ModuleVersionId').disable();
        //this.GeneralForm.get('BodySelectDate').disable();
        //this.GeneralForm.get('ModuleShareId').disable();
        // this.SLCPForm.get('WindowPeriodDate').disable();
        ///this.GeneralForm.get('ModuleYear').disable();
        //this.GeneralForm.get('RequestOfSiteId').disable();
        //this.GeneralForm.get('AccreditationId').disable();
        //this.SLCPForm.get('Remarks').disable();

      }
      var ur;
      ur = window.location.href.split("/")[7];
      var com = [] = ur.split("?")[1];

      if (this.secRoleForm.authAllowed == true) {
        if (com != undefined && com != null) {
          this.btnApproval = true
        }
        else {
          this.btnApproval = false
        }
        this.authorizer = true
      }
      else {
        this.authorizer = false

      }
      if (this.secRoleForm.insertAllowed == true) {

        this.Inserted = true
      }

    })

  }



  disableRecord(): void {

    this.GeneralForm.get('ClientSiteId').disable();
    this.GeneralForm.get('VerificationTypeId').disable();
    this.GeneralForm.get('ProjectTypeId').disable();
    this.GeneralForm.get('StandardId').disable();
    this.GeneralForm.get('AuditTypeId').disable();
    this.GeneralForm.get('SmetaAuditId').disable();
    this.GeneralForm.get('ProductCategory').disable();
    this.GeneralForm.get('Ntnnumber').disable();
    this.GeneralForm.get('SiteProductionProcess').disable();
    this.GeneralForm.get('SiteFactory').disable();
    this.GeneralForm.get('ContractRemarks').disable();
    this.GeneralForm.get('Remarks').disable();
    //this.GeneralForm.get('MethodologyId').disable();
    //this.GeneralForm.get('FromDate').disable();
    //this.GeneralForm.get('ToDate').disable();

    //this.GeneralForm.get('AssessmentCompletedId').disable();
    //this.GeneralForm.get('AssessmentCompletedId').disable();
    // this.SLCPForm.get('CompletedModuleId').disable();
    // this.GeneralForm.get('CompletedStepId').disable();
    // this.GeneralForm.get('TentativeDate').disable();
    //this.GeneralForm.get('NoOfMd').disable();
    // this.GeneralForm.get('ModuleVersionId').disable();
    //this.GeneralForm.get('BodySelectDate').disable();
    //this.GeneralForm.get('ModuleShareId').disable();
    // this.SLCPForm.get('WindowPeriodDate').disable();
    //this.GeneralForm.get('ModuleYear').disable();

    //this.GeneralForm.get('AccreditationId').disable();


  }

  Back(): void {
    this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);

  }

  editProject() {
    debugger
    var ur;
    ur = window.location.href.split("/")[7];
    var com = [] = ur.split("?")[1];
    if (com != undefined && com != null) {
      var Parameter1 = com.split("&")[0];
      var Parameter2 = com.split("&")[1];
      var Parameter3 = com.split("&")[2];


      if (Parameter1.split("=")[0] == "ProjectId") {
        this.ProjectId = Parameter1.split("=")[1];
      }
      else if (Parameter1.split("=")[0] == "StandardId") {
        this.standardId = Parameter1.split("=")[1];
      }
      else if (Parameter1.split("=")[0] == "ClientId") {
        this.ClientId = Parameter1.split("=")[1];
      }

      if (Parameter2.split("=")[0] == "StandardId") {
        this.standardId = Parameter2.split("=")[1];
      }
      else if (Parameter2.split("=")[0] == "ProjectId") {
        this.ProjectId = Parameter2.split("=")[1];
      }
      else if (Parameter2.split("=")[0] == "ClientId") {
        this.ClientId = Parameter2.split("=")[1];
      }


      if (Parameter3.split("=")[0] == "StandardId") {
        this.standardId = Parameter2.split("=")[1];
      }
      else if (Parameter3.split("=")[0] == "ProjectId") {
        this.ProjectId = Parameter2.split("=")[1];
      }
      else if (Parameter3.split("=")[0] == "ClientId") {
        this.ClientId = Parameter3.split("=")[1];
      }
    }
    if (this.ProjectId == 0) {
      this.ProjectId = 0;
      this.btnApproval = false
      //this.savebtn=true
      this.ProjectStatus = "New Project"
    }


    // else {

    //   //this.ClientData();
    // }

    this.OrganizationId = parseInt(localStorage.getItem('organizationId'));
    // this.ClientId = parseInt(localStorage.getItem('clientId'));
    // this.standardId = parseInt(localStorage.getItem('standardId'));
    this.savebtn = true;

  }

  // UpdateProject()
  // {
  //   debugger

  //   this._SlcpService.GetProjectSLCPBYId(this.ProjectId).subscribe(data => {

  //     this.ClientSiteData = data.clientSitesModel;
  //     var ProjectSaData = data.projectGeneralFormModel;
  //     var ClientProjectMod = data.clientProjectModel;


  //     this.standardId = ClientProjectMod.standardId;

  //     this.ClientId = ClientProjectMod.clientId;
  //     this.OrganizationId = ClientProjectMod.agencyId;
  //     this.cllientSiteId=this.ClientSiteData.id
  //     this.GeneralForm.controls.ShiftTimings.setValue(this.ClientSiteData.shiftTimings);
  //     this.GeneralForm.controls.Address.setValue(this.ClientSiteData.address);
  //     this.GeneralForm.controls.ClientSiteId.setValue(this.ClientSiteData.id);
  //     this.GeneralForm.controls.LegalStatus.setValue(this.ClientSiteData.legalStatus)
  //     this.GeneralForm.controls.TotalEmployees.setValue(this.ClientSiteData.totalEmployees)
  //     this.GeneralForm.controls.VerificationTypeId.setValue(ClientProjectMod.verificationTypeId);
  //     if(ClientProjectMod.projectTypeId==1)
  //     {
  //      this.GeneralForm.controls.ProjectTypeId.setValue(ClientProjectMod.projectTypeId);
  //     }
  //     else
  //     {
  //      this.GeneralForm.controls.ProjectTypeId.setValue(0);

  //     }
  //     if (ProjectSaData.applicationDate != null && ProjectSaData.applicationDate != "" && ProjectSaData.applicationDate != undefined && ProjectSaData.applicationDate != NaN) {
  //       let application_Date = new Date(this.datePipe.transform(ProjectSaData.applicationDate, 'yyyy/MM/dd'))

  //       this.GeneralForm.controls.ApplicationDate.setValue(this.datePipe.transform(application_Date, 'yyyy-MM-dd'))
  //      }



  //     //this.GeneralForm.controls.ServicesTypeId.setValue(ProjectSaData.servicesTypeId);

  //     debugger

  //     this.GeneralForm.controls.SiteProductionProcess.setValue(ProjectSaData.siteProductionProcess);
  //     this.GeneralForm.controls.ProductCategory.setValue(ProjectSaData.productCategory);
  //     this.GeneralForm.controls.NTNNumber.setValue(ProjectSaData.nTNNumber);
  //     this.GeneralForm.controls.SiteFactory.setValue(ProjectSaData.siteFactory)
  //     this.ProjectStatus="  "+ClientProjectMod.approvalStatusName;
  //     this.GeneralForm.controls.ApprovalStatusId.setValue(ClientProjectMod.approvalStatusId);
  //     //this.GeneralForm.controls.RequestOfSiteId.setValue(ProjectSaData.requestOfSiteId);

  //     this.GeneralForm.controls.Code.setValue(ClientProjectMod.projectCode);
  //     this.ProjectCode=ClientProjectMod.projectCode

  //     //this.GeneralForm.controls.AccreditationId.setValue(ProjectSaData.accreditationId);

  //     this.loadClientSites();
  //     this._ClientService.GeClientDatabyId(this.ClientId).subscribe((Response) => {
  //       this.clientinfo = Response;


  //     });




  //     if (ClientProjectMod.contractFilePath != null && ClientProjectMod.contractFilePath != undefined && ClientProjectMod.contractFilePath != "") {
  //       this.btnContractDownload = true;

  //     }
  //     if (ClientProjectMod.approvalStatusId == 7 || ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 9 || ClientProjectMod.approvalStatusId == 10) {
  //       this.btnApproval = false;

  //     }
  //     if ( this.secRoleForm.insertAllowed == true) {
  //       if (ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 1) {
  //         this.IsContract = true;

  //       }
  //     }
  //     if (this.secRoleForm.insertAllowed == true) {
  //       if (ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 7 || ClientProjectMod.approvalStatusId == 1 || ClientProjectMod.approvalStatusId == 10 ) {
  //         this.btnContract = true
  //         this.btnContractSave = false
  //         this.GeneralForm.get('ContractStatusId').disable();
  //         this.GeneralForm.controls.ApprovalStatusId.setValue(1);
  //         if (ClientProjectMod.approvalStatusId != 1) {

  //           this.GeneralForm.controls.ContractRemarks.setValue(ClientProjectMod.remarks);
  //           this.GeneralForm.controls.ContractStatusId.setValue(ClientProjectMod.approvalStatusId);

  //         }
  //       }
  //     }
  //     else if (this.authorizer == true) {
  //       if (ClientProjectMod.approvalStatusId == 9 || ClientProjectMod.approvalStatusId == 10) {
  //         this.btnContract = true
  //         this.btnContractSave = true
  //         this.GeneralForm.get('ApprovalStatusId').disable();
  //         this.GeneralForm.get('Remarks').disable();
  //       }
  //     }
  //     if (ClientProjectMod.approvalStatusId == 1 || ClientProjectMod.approvalStatusId == 5) {
  //       this.btnApproval = false
  //     }


  //     if (this.secRoleForm.insertAllowed == true) {
  //       if (ClientProjectMod.approvalStatusId == 5 || ClientProjectMod.approvalStatusId == 6) {
  //         this.savebtn = true
  //       }
  //       else { this.savebtn = false }
  //     }
  //     else {
  //       this.savebtn = false
  //     }
  // if(ClientProjectMod.approvalStatusId == undefined ||ClientProjectMod.approvalStatusId == "" ||ClientProjectMod.approvalStatusId == null ||ClientProjectMod.approvalStatusId == 2 || ClientProjectMod.approvalStatusId == 4)
  // {
  // this.Inserted=false
  // this.btnApproval=true

  // }

  //     if (this.ProjectId > 0 && this.secRoleForm.insertAllowed == true && ClientProjectMod.approvalStatusId == 5 || ClientProjectMod.approvalStatusId == 6) {
  //       this.deletebtn = true;

  //     }
  //     else {
  //       this.deletebtn = false;
  //     }

  //     if (ProjectSaData.applicationFormPath != null && ProjectSaData.applicationFormPath != undefined && ProjectSaData.applicationFormPath != "") {
  //       this.savedownload = true;
  //     }
  //     if (ClientProjectMod.approvalStatusId == 7) {
  //       this.disableRecord()
  //     }

  //     if(this.secRoleForm.insertAllowed == true)
  //     {
  //       this.btnApproval=false;
  //       this.btnContractSave=false;
  //       this.GeneralForm.get('ContractStatusId').disable();
  //       this.GeneralForm.get('ApprovalStatusId').disable();
  //     }
  //     if(this.Revieweruser==true)
  //     {
  //       this.savebtn=false;
  //       this.IsContract=false;
  //       this.deletebtn=false;
  //       this.savebtn=false
  //     }

  //   }


  //   )

  // } 


  // loadServiceType(): void {

  //   this._HiggService.GetALLServicesType().subscribe((Response) => {
  //     this.SerivceTypeList = Response

  //   })
  // }

  UpdateProject() {
    debugger

    this._SlcpService.GetProjectGeneralFormBYId(this.ProjectId).subscribe(data => {

      this.ClientSiteData = data.clientSitesModel;
      var ProjectSaData = data.projectGeneralFormModel;
      var ClientProjectMod = data.clientProjectModel;
      //this.UserAuditList= data

      this.standardId = ClientProjectMod.standardId;

      // this._SlcpService.GetALLModuleVersion(this.standardId).subscribe((Response) => {
      //   this.ModuleVersionList = Response


      // })
      this.ClientId = ClientProjectMod.clientId;
      this.OrganizationId = ClientProjectMod.agencyId;
      this.cllientSiteId = this.ClientSiteData.id
      this.GeneralForm.controls.Address.setValue(this.ClientSiteData.address);
      this.GeneralForm.controls.OutsourceProductionProcessess.setValue(this.ClientSiteData.outsourceProductionProcessess);
      this.GeneralForm.controls.ShiftTimings.setValue(this.ClientSiteData.shiftTimings);
      this.GeneralForm.controls.ClientSiteId.setValue(this.ClientSiteData.id);
      this.GeneralForm.controls.LegalStatus.setValue(this.ClientSiteData.legalStatus)
      this.GeneralForm.controls.TotalEmployees.setValue(this.ClientSiteData.totalEmployees)
      this.GeneralForm.controls.VerificationTypeId.setValue(ClientProjectMod.verificationTypeId);



      if (ClientProjectMod.projectTypeId == 1) {
        this.GeneralForm.controls.ProjectTypeId.setValue(ClientProjectMod.projectTypeId);
      }
      else {
        this.GeneralForm.controls.ProjectTypeId.setValue(0);

      }

      debugger

      this.GeneralForm.controls.Code.setValue(ClientProjectMod.projectCode);
      this.ProjectCode = ClientProjectMod.projectCode

      this.ProjectStatus = "  " + ClientProjectMod.approvalStatusName;
      this.GeneralForm.controls.ApprovalStatusId.setValue(ClientProjectMod.approvalStatusId);

      this.GeneralForm.controls.SmetaAuditId.setValue(ProjectSaData.smetaAuditId);
      this.GeneralForm.controls.StandardId.setValue(ProjectSaData.standardId);
      this.GeneralForm.controls.AuditTypeId.setValue(ProjectSaData.auditTypeId)
      this.GeneralForm.controls.SiteProductionProcess.setValue(ProjectSaData.siteProductionProcess);
      this.GeneralForm.controls.SiteFactory.setValue(ProjectSaData.siteFactory);
      this.GeneralForm.controls.Ntnnumber.setValue(ProjectSaData.ntnnumber);
      this.GeneralForm.controls.ProductCategory.setValue(ProjectSaData.productCategory)


      this.GeneralForm.controls.Remarks.setValue(ProjectSaData.remarks);

      //this.SLCPForm.controls.TentativeDate.setValue(ProjectSaData.tentativeDate);

      //let req = new Date(this.datePipe.transform(ProjectSaData.tentativeDate, 'yyyy/MM/dd'))

      //this.GeneralForm.controls.TentativeDate.setValue(this.datePipe.transform(req, 'yyyy-MM-dd'))

      this.loadClientSites();
      this._ClientService.GeClientDatabyId(this.ClientId).subscribe((Response) => {

        this.clientinfo = Response;
        console.log(this.clientinfo)

      });




      if (ClientProjectMod.contractFilePath != null && ClientProjectMod.contractFilePath != undefined && ClientProjectMod.contractFilePath != "") {
        this.btnContractDownload = true;

      }
      if (ClientProjectMod.approvalStatusId == 7 || ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 9 || ClientProjectMod.approvalStatusId == 10) {
        this.btnApproval = false;

      }


      //  if(ClientProjectMod.approvalStatusId==4 ||ClientProjectMod.approvalStatusId==2 )
      //  {
      //    this.savebtn=false

      //  }
      if (this.secRoleForm.insertAllowed == true) {
        if (ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 1) {
          this.IsContract = true;

        }
      }
      if (this.secRoleForm.insertAllowed == true) {
        if (ClientProjectMod.approvalStatusId == 8 || ClientProjectMod.approvalStatusId == 7 || ClientProjectMod.approvalStatusId == 1 || ClientProjectMod.approvalStatusId == 10) {
          this.btnContract = true
          this.btnContractSave = false
          this.GeneralForm.get('ContractStatusId').disable();
          this.GeneralForm.controls.ApprovalStatusId.setValue(1);
          if (ClientProjectMod.approvalStatusId != 1) {

            this.GeneralForm.controls.ContractRemarks.setValue(ClientProjectMod.remarks);
            this.GeneralForm.controls.ContractStatusId.setValue(ClientProjectMod.approvalStatusId);

          }
        }
        // if (this.authorizer == true && ClientProjectMod.approvalStatusId == 9 || ClientProjectMod.approvalStatusId == 10) {
        //   this.btnContract = true
        //   this.btnContractSave = true
        //   this.SLCPForm.get('ApprovalStatusId').disable();
        //   this.SLCPForm.get('Remarks').disable();
        // }
      }
      else if (this.authorizer == true) {
        if (ClientProjectMod.approvalStatusId == 9 || ClientProjectMod.approvalStatusId == 10) {
          this.btnContract = true
          this.btnContractSave = true
          this.GeneralForm.get('ApprovalStatusId').disable();
          this.GeneralForm.get('Remarks').disable();
        }
      }
      if (ClientProjectMod.approvalStatusId == 1 || ClientProjectMod.approvalStatusId == 5) {
        this.btnApproval = false
      }


      if (this.secRoleForm.insertAllowed == true) {
        if (ClientProjectMod.approvalStatusId == 5 || ClientProjectMod.approvalStatusId == 6) {
          this.savebtn = true
        }
        else { this.savebtn = false }
      }
      else {
        this.savebtn = false
      }
      if (ClientProjectMod.approvalStatusId == undefined || ClientProjectMod.approvalStatusId == "" || ClientProjectMod.approvalStatusId == null || ClientProjectMod.approvalStatusId == 2 || ClientProjectMod.approvalStatusId == 4) {
        this.Inserted = false
        this.btnApproval = true

      }

      if (this.ProjectId > 0 && this.secRoleForm.insertAllowed == true && ClientProjectMod.approvalStatusId == 5 || ClientProjectMod.approvalStatusId == 6) {
        this.deletebtn = true;

      }
      else {
        this.deletebtn = false;
      }

      if (ProjectSaData.applicationFormPath != null && ProjectSaData.applicationFormPath != undefined && ProjectSaData.applicationFormPath != "") {
        this.savedownload = true;
      }
      if (ClientProjectMod.approvalStatusId == 7) {
        this.disableRecord()
      }

      if (this.secRoleForm.insertAllowed == true) {
        this.btnApproval = false;
        this.btnContractSave = false;
        this.GeneralForm.get('ContractStatusId').disable();
        this.GeneralForm.get('ApprovalStatusId').disable();
      }
      if (this.Revieweruser == true) {
        this.savebtn = false;
        this.IsContract = false;
        this.deletebtn = false;
        this.savebtn = false
      }

    }


    )

  }

  loadAllApprovalStatus(): void {
    var roleId = localStorage.getItem('roleId');
    this._SlcpService.GetALLProjectStatus(roleId).subscribe((Response) => {
      this.ApprovalList = Response

    })
  } 


  loadClientSites(): void {
    debugger
    if (this.ClientId > 0 && this.ClientId != null && this.ClientId != undefined) {
      this.ClientSiteList = null;
      this._SlcpService.GetAllClientSites(this.ClientId).subscribe((Response) => {
        this.ClientSiteList = Response
        if (this.cllientSiteId > 0) {
          this.GeneralForm.controls.ClientSiteId.setValue(this.cllientSiteId);
        }
      })
    }
  }

  loadProjectType(): void {

    this._SlcpService.GetAllProjectType().subscribe((Response) => {
      this.ProjectTypeList = Response

    })
  }

  // loadAccreditation(): void {

  //   this._GeneralFormService.GetALLAccreditation().subscribe((Response) => {
  //     this.AccreditationList = Response

  //   })
  // }

  loadVerificationType(): void {

    this._SlcpService.GetAllVerificationType().subscribe((Response) => {
      this.VerificationTypeList = Response

    })
  }

  onSubmit1(): void {
    debugger
    const foData: FormData = new FormData();


    var LoginUserId = localStorage.getItem('userId');


    if (this.ProjectId > 0) {
      foData.append("Id", this.ProjectId.toString());
    }
    else {
      if (this.fileToUpload != null && this.fileToUpload != "" && this.fileToUpload != undefined && this.fileToUpload != undefined && this.fileToUpload != NaN) {


      }
      else {

        abp.message.error("Please Upload Application Form!", "Application Form is required");
        return;
      }
    }
    foData.append('LastModifiedById', LoginUserId);




    foData.append('CreatedById', LoginUserId);
    foData.append('StandardId', this.standardId.toString());
    foData.append('ClientId', this.ClientId.toString());
    foData.append('ApplicationForm', this.fileToUpload);
    Object.keys(this.GeneralForm.controls).forEach(key => {

      if (this.GeneralForm.controls[key].value != null && this.GeneralForm.controls[key].value != "" && this.GeneralForm.controls[key].value != undefined && this.GeneralForm.controls[key].value != NaN) {
        var sname = key;
        //var sname= this.SLCPForm.controls[key].;
        var val = this.GeneralForm.controls[key].value;

        foData.append(sname, val);
      }
    });

    // foData.append('ClientSiteId',this.SLCPForm.get('ClientSiteId').value);
    //  foData.append('VerificationTypeId',this.SLCPForm.get('VerificationTypeId').value);
    //  foData.append('ProjectTypeId',this.SLCPForm.get('ProjectTypeId').value);
    //  foData.append('ServicesTypeId',this.SLCPForm.get('ServicesTypeId').value);
    //  foData.append('MethodologyId',this.SLCPForm.get('MethodologyId').value);
    //  foData.append('FromDate',this.SLCPForm.get('FromDate').value);
    //  foData.append('ToDate',this.SLCPForm.get('ToDate').value);
    //  foData.append('ApplicationDate',this.SLCPForm.get('ApplicationDate').value);
    //  //foData.append('CompletedModuleId',this.SLCPForm.get('CompletedModuleId').value);
    //  foData.append('AccreditationId',this.SLCPForm.get('AccreditationId').value);
    //  foData.append('TentativeDate',this.SLCPForm.get('TentativeDate').value);


    //   foData.append('AssessmentCompletedId',this.SLCPForm.get('AssessmentCompletedId').value);
    // // foData.append('CompletedModuleId',this.SLCPForm.get('CompletedModuleId').value);

    //  foData.append('NoOfMd',this.SLCPForm.get('NoOfMd').value);
    //  foData.append('ModuleVersionId',this.SLCPForm.get('ModuleVersionId').value);
    //  foData.append('BodySelectDate',this.SLCPForm.get('BodySelectDate').value);
    //  foData.append('ModuleShareId',this.SLCPForm.get('ModuleShareId').value);
    //  foData.append('WindowPeriodDate',this.SLCPForm.get('WindowPeriodDate').value);
    //  foData.append('ModuleYear',this.SLCPForm.get('ModuleYear').value);
    //  foData.append('RequestOfSiteId',this.SLCPForm.get('RequestOfSiteId').value);
    //  foData.append('StandardId',this.standardId.toString());
    //  foData.append('CompletedStepId',this.SLCPForm.get('CompletedStepId').value);

    //  foData.append('Remarks',this.SLCPForm.get('Remarks').value);
    //foData.append('ApplicationForm',this.fileToUpload);



    this._SlcpService.ProjectgeneralformCreateWithFile(foData).subscribe((Response) => {

      //abp.message.info(Response.message)

      //this.router.navigateByUrl('/app/pages/sales/all-projects?'+this.ClientId);
      if (Response.message == '1') {
        abp.message.info("Successfully Saved!")
        this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);
      }
      else if (Response.message == '2') {
        abp.message.info("Successfully Updated!")
        this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);
      }
      else if (Response.message == '0') {
        abp.message.error("Not Inserted!")
      }

    })
  }
  id: number

  onTableSizeChange(event): void {
    this.pagedDto.pageSize = event.target.value;

  }
  editRecord(e) {

    // var userId=item;
    var urlink = e;
    this.router.navigateByUrl(e + this.ClientId)

  }




  handlefileInput(e: any) {

    this.fileToUpload = <File>e?.target?.files[0];
    //this.url=e.target.value; 


  }
  onOptionsSelected(id: number) {
    console.log("the selected value is " + id);
  }

  DiscountTypeChange($event) {

    abp.message.info("the selected value is " + $event.value);
  }

  onChange(deviceValue) {

    this._ClientService.GetSiteById(deviceValue).subscribe((Response) => {

      this.GeneralForm.controls.Address.setValue(Response.address);
      this.GeneralForm.controls.OutsourceProductionProcessess.setValue(Response.outsourceProductionProcessess);
      this.GeneralForm.controls.ShiftTimings.setValue(Response.shiftTimings);

      this.GeneralForm.controls.LegalStatus.setValue(Response.legalStatus)
      this.GeneralForm.controls.TotalEmployees.setValue(Response.totalEmployees)


    })
  }

  OnApprovalSubmit(): void {



    var LoginUserId = localStorage.getItem('userId');



    const ClientSitesModel =

    {

      Id: this.ProjectId,
      ProjectId: this.ProjectId,
      ApprovalStatusId: this.GeneralForm.get('ApprovalStatusId').value,
      Remarks: this.GeneralForm.get('Remarks').value,
      RemarksById: LoginUserId.toString(),

    };


    this._SlcpService.ApprovedGeneralProject(ClientSitesModel).subscribe((Response) => {

      abp.message.info(Response.message)
      //this.reloadGrid();
      this.router.navigateByUrl('app/pages/master-setups/dashboard');

    })
  }

  DownloadApplicationForm(): void {


    // this.ProjectId=e.row.data.id;
    // var fillename=e.row.data.title;
    var fillename = "Document File";
    this._GeneralFormService.downloadApplicationForm(this.ProjectId).subscribe((result: Blob) => {
      const Blb = new Blob([result], { type: result.type });
      // const url=window.URL.createObjectURL(Blb);
      // window.open(url);
      // console.log("success");


      const a = document.createElement('a');
      a.setAttribute('style', 'display:none;');
      document.body.appendChild(a);
      // a.download =fillename;  
      // const fileName =

      //="farooq";
      a.href = URL.createObjectURL(Blb);
      a.target = '_blank';
      a.click();
      document.body.removeChild(a);

    })
  }

  delete(): void {

    if (this.ProjectId != 0 && this.ProjectId != null && this.ProjectId != undefined) {

      abp.message.confirm((""),
        undefined,
        (result: boolean) => {
          if (result) {
            // this.SecUserService.Deleteuser(e.row.data.id).subscribe() 
            //     abp.message.info("Deleted successfully", "Status", {});

            this._GeneralFormService.DeleteProject(this.ProjectId).subscribe((Response) => {

              abp.message.info(Response.message)

              this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);
            })

          }
        }
      )

    }
  }


  // loadRequestOfSite(): void {

  //   this._GeneralFormService.GetALLRequestOfSite().subscribe((Response) => {
  //     this.RequestOfSiteList = Response


  //   })
  // }

  loadStatus(): void {

    const item = {
      id: 7,
      name: 'Contract Approved',


    };
    this.ContractApprovalList.push(item);
    const item2 = {
      id: 8,
      name: 'Contract Reject',


    };
    this.ContractApprovalList.push(item2);

  }

  // loadstandard(): void {

  //   this.LibraryResourceService.getAllCertification().subscribe((Response)=>{
  //     this.CertificationList = Response
  //       this.CertificationList.push({id: 0 , name: "---- ALL----" })
  //       this.CertificationList.reverse();
  //   })
  // }

  loadAllCertification(): void {
    this.LibraryResourceService.getAllCertification().subscribe((Response) => {
      this.CertificationList = Response
    })
  }

  loadAuditTypeId(): void {

    this._UserStandardService.GetAllAuditType().subscribe((Response) => {
      this.AuditTypeList = Response

    })
  }

  loadSmetaAudit(): void {
    debugger
    this._GeneralFormService.GetAllSmetaAuditPillars().subscribe((Response) => {
      this.SmetaAuditList = Response
      console.log(Response);


    })
  }
  Pdf() {
    let Today_Date = new Date(this.datePipe.transform(Date.now(), 'yyyy/MM/dd'))

    let TodayDate = this.datePipe.transform(Today_Date, 'yyyy-MM-dd')
    //let completestep = this.CompletedSetupList.find(x => x.id == this.GeneralForm.get('CompletedStepId').value)
    //let methodology = this.MethodologyList.find(x => x.id == this.GeneralForm.get('MethodologyId').value)

    let clientSite = this.ClientSiteList.find(x => x.id == this.GeneralForm.get('ClientSiteId').value)
    //var clientinfo;

    // Font bold = Font font = new Font(FontFamily.HELVETICA, 12, Font.BOLD);
    // Phrase p = new Phrase("NAME: ", bold);
    //   this._ClientService.GeClientDatabyId(this.ClientId).subscribe((Response) => 
    // {

    //   this.clientinfo=Response;    


    // });

    var documentDefinition = {
      info: {
        title: 'GeneralForm Proposal Weaving' + "/" + this.clientinfo.name,

      },
      // pageMargins: [ 10, 70, 10, 20 ],
      pageSize: 'A4',
      pageMargins: [50, 50, 50, 50],
      background: [
        {
          image: 'data:image/jpeg;base64,',

          width: 500,
          opacity: 0.2,
          margin: [20, 280, 0, 0]
        }
      ],
      content: [

        {
          columns: [




            { text: 'Code: ' + this.GeneralForm.get('Code').value + '\n' + 'Date: ' + TodayDate, },


            {
              image: 'data:image/jpeg;base64,', alignment: "center",
              margin: [0, -30, 0, -10],
              width: 150,
              height: 120,



            },

          ]
        },
        {
          text: 'OZONE SUSTAINABILITY',
          alignment: "center",
          fontSize: 16,
          //decoration: "underline" ,
          bold: true,


          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 10]
        },

        { canvas: [{ type: 'line', x1: 160, y1: 0, x2: 335, y2: 0, lineWidth: 3 }] },

        {
          text: 'PROPOSAL',
          alignment: "center",

          bold: true,

          margin: [0, 7, 0, 10]
        },


        // {
        //   text: 'OZONE SUSTAINABILITY',
        //   alignment: "center",
        //   fontSize: 16,
        //   //decoration: "underline" ,
        //   bold : true,


        //   //textTransform: 'uppercase',
        //   // background: 
        //   // 'assets/img/ozone-group-logo.jpg',



        //   margin: [0, 0, 0, 10]
        // },

        // {canvas: [ { type: 'line', x1: 170, y1: 0, x2: 345, y2: 0, lineWidth: 3 } ]},

        // {
        //   text: 'PROPOSAL',
        //   alignment: "center",

        //   bold : true,

        //   margin: [0, 7, 0, 10]
        // },


        // {

        //   table: {
        //     widths: [500],
        //     heights: [20],
        //     body: [
        //       // ['row 1 with height 20'],
        //       // ['row 2 with height 50'],
        //       [{
        //         text: 'OZONE SUSTAINABILITY'+"\n"+ "________________________"+'PROPOSAL',

        //         color: 'white',
        //         fillColor: '#4CAF50',
        //         border: [false, true, false, true],
        //         margin: [0, 5, 0, 5],
        //         textTransform: 'uppercase',
        //         alignment: "center",
        //         fontSize: 16,
        //         //decoration: "underline" ,
        //         bold : true,
        //       },

        //     ]
        //     ]
        //   }
        // },
        {
          text: 'Company Representative',
          alignment: "left",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,


          textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 20, 0, 0]
        },

        { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 495, y2: 0, lineWidth: 1 }] },

        {
          text: this.clientinfo.contactPerson + "  (" + this.clientinfo.position + ")",
          //alignment: "left",
          fontSize: 11,
          //decoration: "underline" ,
          //bold : true,
          margin: [0, 5, 0, 0]

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },
        {
          text: this.clientinfo.name,
          //alignment: "left",
          fontSize: 11,
          //decoration: "underline" ,
          //bold : true,
          margin: [0, 0, 0, 0]

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },
        {
          text: this.clientinfo.address1,
          //alignment: "left",
          fontSize: 11,
          //decoration: "underline" ,
          //bold : true,
          margin: [0, 0, 0, 0],
          lineHeight: 1.5,
          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },

        //   {
        //     canvas: [ {  type: 'line', x1: 0, y1: 0, x2: 500, y2: 0, lineWidth: 3 } ]

        // },
        {
          text: 'SUB: CERTIFICATION IN STANDARDS:',
          alignment: "left",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,

          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },
        {
          canvas: [{ type: 'line', x1: 0, y1: 0, x2: 495, y2: 0, lineWidth: 1 }]

        },

        {
          text: 'SLCP VERIFICATION',
          alignment: "left",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,
          margin: [0, 5, 0, 0]
          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },
        {
          text: 'Thank you for choosing us for taking an opportunity to present us certification of SLCP verification in your esteemed organization.' + '\n'
            + 'Among the range of certifications, The Ozone has accredited scheme certification of SA8000:2014 thru SAAS. We have approval as HIG Index Verifier Body and Social Labour Convergence Program (SLCP) as Verifier Body in Environmental and Social Assessments in Industry. We are doing assessments and '
            + 'certifications in ISO 9001:2015 for Quality Management Systems, ISO 14001:2015 for Environmental '
            + 'Management Systems, ISO 45001:2018 Occupational Health and Safety Management System, ISO '
            + '22000:2018 FSMS, ISO 18001: OHSAS for Health & Safety, HACCP and ISO 13485:2016.' + '\n'
            + 'The Ozone fosters a ‘listening philosophy ‘to be administered by its full support before and after the '
            + 'certification by working with you, as you further improve your systems, streamline your processes & '
            + 'strengthen your position as a world class organization. Also, as a value addition various training programs '
            + 'and workshops are conducted to enhance better understanding of the standard and imbibing it to '
            + 'seamless and integrated organizational development.' + '\n'
            + 'We are enclosing our proposal for certification with SLCP Verification with all details.' + '\n'
            + 'We are pleased to respond your queries in this regard.' + '\n'
            + 'Regards' + '\n'
            + 'Ozone Sustainability Management System' + '\n'
            + 'Pakistan',
          fontSize: 12,
          alignment: 'justify',
          margin: [0, 11, 0, 0]
          //decoration: "underline" ,
          //bold : true,

          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },

        {
          margin: [350, 0, 0, 0],
          pageBreak: 'before',
          columns: [


            {
              image: 'data:image/jpeg;base64,', alignment: "center",
              margin: [0, -40, 0, 0],
              width: 150,
              height: 100,


            },

          ]
        },
        // {
        //   text: 'OZONE SUSTAINABILITY',
        //   alignment: "left",
        //   fontSize: 16,
        //   //decoration: "underline" ,
        //   bold: true,
        //   pageBreak: 'before',

        //   //textTransform: 'uppercase',
        //   // background: 
        //   // 'assets/img/ozone-group-logo.jpg',



        //   margin: [0, 10, 0, 0]
        // },

        // {canvas: [ { type: 'line', x1: 170, y1: 0, x2: 345, y2: 0, lineWidth: 3 } ]},

        // {
        //   text: 'PROPOSAL',
        //   alignment: "left",

        //   bold: true,

        //   margin: [0, 3, 0, 10]
        // },

        {
          text: 'ORGANIZATION DETAILS:',
          alignment: "left",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,

          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          //margin: [0, 0, 0, 0]
        },
        {
          canvas: [{ type: 'line', x1: 0, y1: 0, x2: 495, y2: 0, lineWidth: 1 }]

        },
        {
          //alignment: 'justify',
          columns: [

            {
              width: 200,
              text: 'Total no of site:',
              bold: true,
              margin: [0, 5, 0, 0]
            },
            {
              text: this.clientinfo.siteCount,
              margin: [0, 5, 0, 0]
            },
          ],


        },
        {
          columns: [

            {
              width: 200,
              text: 'Number of employees:',
              bold: true,
              margin: [0, 2, 0, 0]
            },
            {
              text: this.GeneralForm.get('TotalEmployees').value,
              margin: [0, 2, 0, 0]
            },
          ],
        },
        {
          columns: [

            {
              width: 200,
              text: 'Completed Step:',
              bold: true,
              margin: [0, 2, 0, 0]
            },
            {
              //text: completestep.name,
              margin: [0, 2, 0, 0]
            },
          ],
        },
        {
          columns: [

            {
              width: 200,
              text: 'Methodology:',
              bold: true,
              margin: [0, 2, 0, 0]
            },
            {
              //text: methodology.name,
              margin: [0, 2, 0, 0]
            },
          ],
        },

        {
          text: 'ORGANIZATION SITES COVERED UNDER CERTIFICATION',
          alignment: "center",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,


          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 30, 0, 10]
        },

        {
          text: 'SITE TO BE AUDITED: ',
          alignment: "left",
          fontSize: 12,
          decoration: "underline",
          bold: true,
          width: 50,

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 5, 0, 0]
        },
        {
          text: clientSite.siteName + ",  " + clientSite.address,
          //alignment: "left",
          fontSize: 11,

          //bold : true,


          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 5, 0, 0]
        },

        // {
        //   columns: [

        //     {	
        //       width: 120,
        //       text: 'SITE TO BE AUDITED: ',
        //       bold: true,
        //       margin: [0, 5, 0, 0],
        //       alignment: "left",
        //      fontSize: 12,


        //     },
        //     {
        //       text: this.clientinfo.name +",  "+ this.clientinfo.address1 ,
        //       margin: [0, 5, 0, 0],
        //       fontSize: 11,
        //     },
        //   ],
        // },
        //   {
        //     canvas: [ {  type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1 } ]

        // },

        {
          text: 'COMMERCIAL ASPECTS OF PROPOSAL',
          alignment: "left",
          fontSize: 14,
          //decoration: "underline" ,
          bold: true,

          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 10, 0, 0]
        },
        {
          canvas: [{ type: 'line', x1: 0, y1: 0, x2: 495, y2: 0, lineWidth: 1 }]

        },

        {
          //alignment: 'justify',
          columns: [

            {
              width: 100,
              text: 'Dated:',
              bold: true,
              margin: [0, 5, 0, 0]
            },
            {
              text: '24-11-2021',
              margin: [0, 5, 0, 0]
            },
          ],


        },
        {
          //alignment: 'justify',
          columns: [

            {
              width: 100,
              text: 'Attention:',
              bold: true,
              margin: [0, 0, 0, 0],
              fontSize: 11,
            },
            {
              text: this.clientinfo.contactPerson + "  (" + this.clientinfo.position + ")",
              margin: [0, 0, 0, 0],
              fontSize: 11,
            },
          ],


        },
        {
          //alignment: 'justify',
          columns: [

            {
              width: 100,
              text: 'Client Name:',
              bold: true,
              margin: [0, 0, 0, 0],
              fontSize: 11,
            },
            {
              text: this.clientinfo.name,
              //alignment: "left",
              fontSize: 11,
              margin: [0, 0, 0, 0]
            },
          ],


        },
        {
          //alignment: 'justify',
          columns: [

            {
              width: 100,
              text: 'Address:',
              bold: true,
              margin: [0, 0, 0, 0],
              fontSize: 11,
            },
            {
              text: this.clientinfo.address1,
              //alignment: "left",
              fontSize: 11,
              margin: [0, 0, 0, 0]
            },
          ],


        },

        '\n',
        '\n',
        {

          table: {
            heights: [40, 25, 25],
            // widths: [370, 120],
            widths: ['75%', '25%'],
            margin: [0, 10, 0, 0],
            //alignment: "center",
            body: [
              [{
                text: 'Service Type', alignment: 'center', color: 'white', margin: [0, 10, 0, 0],
                fillColor: '#4258ad',
              }, {
                text: 'Charges' + '\n' + 'USD/-', alignment: 'center', color: 'white',
                fillColor: '#4258ad', margin: [0, 3, 0, 0],
              }],
              [{ text: 'SLCP Verification Charges + SLCP Fee  ', fontSize: 11, margin: [0, 3, 0, 0], }, ''],
              [{ text: 'TOTAL + GST', fontSize: 11, bold: true, margin: [0, 3, 0, 0], }, '']
            ],
            //alignment: "center",
          },
          layout: {
            hLineWidth: function (i, node) {
              return (i === 0 || i === node.table.body.length) ? 2 : 1;
            },
            vLineWidth: function (i, node) {
              return (i === 0 || i === node.table.widths.length) ? 2 : 1;
            },
            hLineColor: function (i, node) {
              return 'black';
            },
            vLineColor: function (i, node) {
              return 'black';
            },
            hLineStyle: function (i, node) {
              if (i === 0 || i === node.table.body.length) {
                return null;
              }
              return { dash: { length: 4, space: 4 } };
            },
            vLineStyle: function (i, node) {
              if (i === 0 || i === node.table.widths.length) {
                return null;
              }
              return { dash: { length: 4 } };
            },
            // paddingLeft: function(i, node) { return 4; },
            // paddingRight: function(i, node) { return 4; },
            // paddingTop: function(i, node) { return 2; },
            // paddingBottom: function(i, node) { return 2; },
            // fillColor: function (i, node) { return null; }
          }
        },
        {
          margin: [350, 0, 0, 0],
          pageBreak: 'before',
          columns: [


            {
              image: 'data:image/jpeg;base64,', alignment: "center",
              margin: [0, -40, 0, 0],
              width: 150,
              height: 100,


            },

          ]
        },
        // {
        //   text: 'OZONE SUSTAINABILITY',
        //   alignment: "left",
        //   fontSize: 16,
        //   //decoration: "underline" ,
        //   bold: true,
        //   pageBreak: 'before',

        //   //textTransform: 'uppercase',
        //   // background: 
        //   // 'assets/img/ozone-group-logo.jpg',



        //   margin: [0, 10, 0, 0]
        // },

        // {canvas: [ { type: 'line', x1: 170, y1: 0, x2: 345, y2: 0, lineWidth: 3 } ]},

        // {
        //   text: 'PROPOSAL',
        //   alignment: "left",

        //   bold: true,

        //   margin: [0, 3, 0, 10]
        // },

        {
          text: 'Client organization’s Signature means validation and acceptance of proposal by Applicant client Organization for the Certification/Verification Scheme(s) mentioned in proposal.',
          ul: [
            'Client organization’s Signature means validation and acceptance of proposal by Applicant client Organization for the Certification/Verification Scheme(s) mentioned in proposal.',

          ],
          alignment: 'justify',
          fontSize: 11,
          //decoration: "underline" ,
          //bold : true,

          //pageBreak: 'before',

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 20, 0, 0]
        },
        {
          text: 'Terms & Conditions:',
          alignment: "left",
          fontSize: 12,
          decoration: "underline",
          bold: true,
          width: 50,

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 5, 0, 0],
        }, {
          ul: [
            'The prices quoted in USD and the Invoices will be issued in Local Currency: USD. The rate of convergence will be applied at current rates on the invoice day. ',
            'Should any changes occur within your organization, Ozone Group reserves the right to reissue the quotation. Examples of changes affecting the quote are an increase in the number of employees, a change in the scope of registration (i.e. addition of a product line) etc.',
            'Our quotation is valid for acceptance for 30 days (Thirty Days) from the quotation date.',

          ],
          alignment: 'justify',
          fontSize: 11,
          margin: [10, 2, 0, 0],
        },
        {
          text: 'THE Ozone GROUP reserves the right to accept / reject an “Acceptance of Proposal” submitted outside of the proposal validity period.',
          alignment: 'justify',
          fontSize: 11,
          // decoration: "underline" ,
          // bold : true,
          //width:50,

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 10, 0, 0],
        },
        {
          text: 'ACCEPTANCE OF QUOTE:',
          alignment: "left",
          fontSize: 12,
          decoration: "underline",
          bold: true,


          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 5, 0, 0],
        },
        {
          type: 'none',
          ul: [
            'If this quotation is acceptable by Applicant Client Organization, then please send a confirmation by email referencing the quotation number and attach the signed and stamped copy of this quotation.',
            '\n' +
            'By signing this quotation, it means the client organization is accepting this quotation and follow “we confirm the information contained above is correct and agree to comply with the OSMS (Ozone Group) Scheme Rules and agree to pay all fees relating to the provision of certification/Verification services.”',

          ],
          fontSize: 11,
          alignment: 'justify',


        },
        {
          text: 'The organization shall update Ozone Group if there are any changes in legal status, organization and management including number of employees, addresses and sites, scope of operation and major changes to the management system / processes.',
          //alignment: "left",
          fontSize: 11,
          alignment: 'justify',
          // decoration: "underline" ,
          // bold : true,
          //width:50,

          //textTransform: 'uppercase',
          // background: 
          // 'assets/img/ozone-group-logo.jpg',



          margin: [0, 10, 0, 0],
        },

        // {
        //   columns: [


        //     { canvas: [{ type: 'line', x1: 0, y1: 90, x2: 150, y2: 90, lineWidth: 1, }], width: 370 },

        //     { canvas: [{ type: 'line', x1: 0, y1: 90, x2: 150, y2: 90, lineWidth: 1 }] },
        //   ],
        // },
        {
          margin: [0, 130, 0, 0],
          columns: [


            { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 150, y2: 0, lineWidth: 1, }], width: 340 },

            { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 150, y2: 0, lineWidth: 1 }] },
          ],
        },
        {
          columns: [

            {
              text: 'Ozone Group',
              alignment: "left",
              fontSize: 11,
              //decoration: "underline" ,
              bold: true,
              width: 340,
              margin: [0, 5, 0, 0],
              color: '#338fdb',
            },
            {
              text: 'CLIENT DETAILS',
              alignment: "left",
              fontSize: 11,
              //decoration: "underline" ,
              bold: true,
              margin: [0, 5, 0, 0],
              color: '#338fdb',
            },

          ],
        },

        {
          columns: [

            {
              text: 'Zahid Iqbal' + '\n' + 'Operation Head' + '\n' + 'Cell #: +92-3333-2179917',
              alignment: "left",
              fontSize: 10,
              //decoration: "underline" ,
              bold: true,
              width: 340,
              margin: [0, 5, 0, 0],
            },
            {
              text: this.clientinfo.contactPerson + '\n' + this.clientinfo.position + '\n' + 'Cell #:' + this.clientinfo.mobileNumber + '\n' + 'Email:' + this.clientinfo.email,
              alignment: "left",
              fontSize: 10,
              //decoration: "underline" ,
              bold: true,
              margin: [0, 5, 0, 0],
            },

          ],
        },

        // {
        //   columns: [




        //   ],
        // },





      ]


    }

    const pdfFile = pdfmake.createPdf(documentDefinition, null, null, pdfFonts.pdfMake.vfs);
    pdfFile.open();
    //pdfFile.download('SLCP Proposal Weaving.pdf');
    //pdfFile.download();
    //pdfmake.createPdf(documentDefinition).open();
    //pdfmake.createPdf(documentDefinition).download();
  }

  onContractSubmit(): void {
    try {
      var fileName = this.fileToUpload.name.split("_")[0];

      if (fileName == this.ProjectCode) {

        if (this.fileToUpload == null || this.fileToUpload == undefined || this.fileToUpload == "" || this.fileToUpload == '' || this.fileToUpload == NaN) {

          abp.message.error("Contract file is required", "Please Upload Contract File");
          return;
        }


        var LoginUserId = localStorage.getItem('userId');
        const foData: FormData = new FormData();

        if (this.ProjectId > 0) {
          foData.append("Id", this.ProjectId.toString());
        }
        foData.append('LastModifiedById', LoginUserId);

        foData.append('ContractForm', this.fileToUpload);
        foData.append('Remarks', this.GeneralForm.get('ContractRemarks').value);
        // foData.append('ApprovalStatusId',this.SLCPForm.get('ContractStatusId').value);


        this._GeneralFormService.ContarctsSubmit(foData).subscribe((Response) => {

          abp.message.info(Response.message)

          this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);

        })
      }
      else {
        //this.SLCPForm.controls.File.setValue("");
        abp.message.error("File Name Is Incorrect, Project code must be include in File name. File name must be  " + this.ProjectCode + "_Contract File", "Please Upload Correct File")
      }

    }
    catch (error) {
      // this.SLCPForm.controls.File.setValue("");
      abp.message.error("File Name Is Incorrect, Project code must be include in File name. File name must be  " + this.ProjectCode + "_Contract File", "Please Upload Correct File")

    }

    console.log(this.fileToUpload.name)



  }

  contractApproved(): void {

    if (this.GeneralForm.get('ContractStatusId').value == null || this.GeneralForm.get('ContractStatusId').value == undefined || this.GeneralForm.get('ContractStatusId').value == "" || this.GeneralForm.get('ContractStatusId').value == '' || this.GeneralForm.get('ContractStatusId').value == NaN) {

      abp.message.error("Contract Status is required", "Please Select Status");
      return;
    }

    var LoginUserId = localStorage.getItem('userId');
    const foData: FormData = new FormData();

    if (this.ProjectId > 0) {
      foData.append("Id", this.ProjectId.toString());
    }
    foData.append('LastModifiedById', LoginUserId);


    foData.append('Remarks', this.GeneralForm.get('ContractRemarks').value);
    foData.append('ApprovalStatusId', this.GeneralForm.get('ContractStatusId').value);


    this._SlcpService.ContractApproval(foData).subscribe((Response) => {

      debugger
      //abp.message.info(Response.message)
      if (Response.message == '1') {
        abp.message.info("Successfully Saved!")
        this.router.navigateByUrl('app/pages/master-setups/dashboard');

      }
      else if (Response.message == '2') {
        abp.message.info("Project amount not set in Project Amount form for this Standard!")
        ///this.router.navigateByUrl('/app/pages/sales/all-projects?'+this.ClientId);
      }
      else if (Response.message == '0') {
        abp.message.error("Not Inserted!")
      }
      // this.router.navigateByUrl('/app/pages/sales/all-projects?' + this.ClientId);

    })
  }



  contractDownload(): void {


    // this.ProjectId=e.row.data.id;
    // var fillename=e.row.data.title;
    var fillename = "Document File";
    this._SlcpService.downloadContract(this.ProjectId).subscribe((result: Blob) => {
      const Blb = new Blob([result], { type: result.type });
      // const url=window.URL.createObjectURL(Blb);
      // window.open(url);
      // console.log("success");


      const a = document.createElement('a');
      a.setAttribute('style', 'display:none;');
      document.body.appendChild(a);
      // a.download =fillename;  
      // const fileName =

      //="farooq";
      a.href = URL.createObjectURL(Blb);
      a.target = '_blank';
      a.click();
      document.body.removeChild(a);

    })
  }

  dashboard(): void { this.router.navigateByUrl('/app/home'); }

  //    loadAgency(): void {
  //   this._GeneralFormService.getAllServices().subscribe((Response) => {
  //     this.GeneralList = Response
  //   })
  // }
}
