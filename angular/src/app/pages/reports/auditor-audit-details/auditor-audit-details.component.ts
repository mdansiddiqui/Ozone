  ...
  onExporting(e) {
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Audit Report.xlsx');
      });
    });
    e.cancel = true;
  }
...
const obj = this.auditorReportDetailsList.find(item => item.id === e.row.data.id);
console.log(obj);
...
if (this.QcCommentsForm.get('QcStatusId').value != this.QcCommentsForm.get('QcStatusId').value) {
  ...
  const obj = this.auditorReportDetailsList.find(item => item.id === this.QcCommentsId && item.qcdocumentsListId===this.QcdocumentsListId);
    ...
    (result: boolean) => {
      if (result) {
        ...
        for(let i = 0; i < this.auditorReportDetailsList.length; ++i){
          if (this.auditorReportDetailsList[i].id === e.row.data.id && this.auditorReportDetailsList[i].qcdocumentsListId ===e.row.data.qcdocumentsListId) {
    ...
    return e.row.isEditing;
     ...
     // const source = `data:application/pdf;base64,${e.row.data.documentContent}`;
      ...
      DownloadViaible(e) {
        ...
        if(e.row.data.documentName!=null &&  e.row.data.documentName!=undefined && e.row.data.documentName!=isNaN && e.row.data.documentName!="" && e.row.data.documentName!='')
        {
          return !e.row.isEditing;
    }
    else
